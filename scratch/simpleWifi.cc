...
int main (int argc, char *argv[])
{
  ...
  Ptr<OutputStreamWrapper> ip_stream = asciiTraceHelper.CreateFileStream (logName);

  // Convert to time object
  Time interPacketInterval = Seconds (interval);

  // disable fragmentation for frames below 10k bytes
  Config::SetDefault ("ns3::WifiRemoteStationManager::FragmentationThreshold", StringValue ("10000"));
  // turn off RTS/CTS for frames below 10k bytes
  ...
  InetSocketAddress remote = InetSocketAddress ("255.255.255.255", 80);
  ...
  // Tracing
  wifiPhy.EnablePcap ("wifi-simple-infra", devices.Get(0), true);

  // Output what we are doing
  NS_LOG_UNCOND ("Testing " << numPackets  << " packets sent with receiver rss " << rss );
  *ip_stream->GetStream() << i.GetAddress(0) << std::endl;
