...
HybridBuildingsPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::HybridBuildingsPropagationLossModel")
    ...
    .AddAttribute ("HewLossInternalWalls",
                   "Loss due to internall walls in the HEW building",
                   DoubleValue (12.0),
    ...
    .AddAttribute ("hewScenario",
                   ...
                   BooleanValue (false),
                   MakeBooleanAccessor (&HybridBuildingsPropagationLossModel::m_hewScenario),
                   MakeBooleanChecker ())
    ;
...
HybridBuildingsPropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  ...
  if (m_hewScenario)
    {
      NS_ASSERT (a1->IsIndoor () == b1->IsIndoor ());
      NS_ASSERT (a1->GetBuilding () == b1->GetBuilding ());
      loss = HewScenarioInternallWallsLoss (a1, b1);
      NS_LOG_DEBUG ("Overwritting losses to comply with HEW scenarios: " << loss);
    }
...
double
HybridBuildingsPropagationLossModel::HewScenarioInternallWallsLoss (Ptr<MobilityBuildingInfo> a, Ptr<MobilityBuildingInfo>b) const
{
...
bool 
HybridBuildingsPropagationLossModel::GetHewScenario ()
{
  return m_hewScenario;
}
...
double 
HybridBuildingsPropagationLossModel::GetFrequency ()
{
  return m_frequency;
}
